...
class Clickbot:
    ...
    def __init__(self, profile_dir):
        ...
        self.european_wheel = [0, 32, 15, 19, 4, 21, 2, 25, 17, 34, 6, 27, 13, 36, 11, 30, 8, 23, 10, 5, 24, 16, 33, 1,
                               20, 14, 31, 9, 22, 18, 29, 7, 28, 12, 35, 3, 26]
        self.jump_array = list(range(0, 19)) + list(range(-18, 0))
        self.profile_dir = profile_dir
        self.rotor_isolation_scatter = None
        self.peak_start_idxs = None
    ...
    def set_jump_values(self):
        ...
        print(
            """When selecting jump values, start with the left edge of the peak and go right.
            For example, to capture values -8,-7,-6...0,1,2,3,4, do -8 to 4")""")
        while True:
            try:
                self.jump_anti = []
                    ...
                    for ball_rev_range in ball_rev_scatter.keys():
                        if direction == "a":
                            if ball_rev_range == self.ball_revs_anti:
                                return self.get_tuned_given_jumps(ball_rev_scatter[ball_rev_range]["anticlockwise"],
                                                                  raw_prediction)
                        else:
                            if ball_rev_range == self.ball_revs_clock:
                                return self.get_tuned_given_jumps(ball_rev_scatter[ball_rev_range]["clockwise"],
                                                                  raw_prediction)
