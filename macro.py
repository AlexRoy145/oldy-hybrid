...
class Macro:
    ...
    def is_screen_condition_true(self, green=False):
        sct_img = self.sct.grab(
            {"left": self.screen_condition[0], "top": self.screen_condition[1], "width": 1, "height": 1, "mon": 0})
    ...
    def record_signin_macro(self):
        input(
            f"Record the signin macro. Start by closing the currently open browser, then opening the browser on the \
            desktop. Perform all the steps necessary to bring up the Firefox web browser and click inside the URL bar. \
            Then click back here after you're done. Press ENTER to start: ")
        mouse_listener = mouse.Listener(on_click=self.on_click)
        mouse_listener.start()
        self.macro = []
        self.macro_start_time = time.perf_counter()
        ...
        # insert the URL entry event into the macro events. At this point, the last event was clicking inside the URL
        # bar.
        self.macro.append(self.MacroEvent(None, None, .5, None, "site"))

        input(
            f"At this time, put the site URL into the URL bar. Then perform all the steps necessary to bring up the log\
             in prompt for the website, starting with clicking the go button to go to the website. After that, click in\
              the username box, then click in the password box, then click back into this window. \
              Press ENTER to start: ")
        mouse_listener = mouse.Listener(on_click=self.on_click)
        mouse_listener.start()
        self.macro_start_time = time.perf_counter()
        input(
            f"Press ENTER in this window once you finished clicking inside the username box, and then the password \
            box: ")
        mouse_listener.stop()
        ...
        # now we need to insert the username/password into the macro events. at this point, the last two events were
        # clicks in the username box and password box
        self.macro.insert(-1, self.MacroEvent(None, None, .5, None, "username"))

        self.macro.append(self.MacroEvent(None, None, .5, None, "password"))

        input(
            f"At this time, put the correct username and password into the log in prompt for the website so you're \
            ready to sign in. Press ENTER to start recording the rest of the macro, starting with clicking the log \
            in button and ending with the betting interface pulled up: ")
        mouse_listener = mouse.Listener(on_click=self.on_click)
        mouse_listener.start()
        self.macro_start_time = time.perf_counter()
