...
K: Type out the most recent ball timings to the anydesk window. Delay is 2 seconds, so once you hit enter, you have 2 
   seconds before it starts typing.
...
"""
            ...
            if choice == "help":
                ...
                if self.direction == "anticlockwise":
                    ...
                    new_direction = "cw"
                self.scatter.add_data(new_direction, self.raw, winning, self.rotor_speed, None, None)
                self.scatter.save_profile(SCATTER_DATA_FILE)
                continue
            elif choice == "e":
                ...
                for anti_spin in anti_spins:
                    print(
                        f"Diamond hit: {anti_spin.diamond_hit}, Ball revs: {anti_spin.ball_revs}, Rotor speed\
                        : {anti_spin.rotor_speed}")

                print("\nMost recent CLOCKWISE spin data (newest spins listed first):")
                for clock_spin in clock_spins:
                    print(
                        f"Diamond hit: {clock_spin.diamond_hit}, Ball revs: {clock_spin.ball_revs}, Rotor speed\
                        : {clock_spin.rotor_speed}")
                ...
                self.raw_adjustment = int(input(
                    "Input the new raw adjustment (example, +4 to shift the raw clockwise 4 pockets, -4 to shift the \
                    raw anti 4 pockets): "))
                continue
            elif choice == "ss":
                # '''
                ...
                self.scatter.graph(direction=direction_data, rotor_speed_range=rotor_speed_range,
                                   fall_point_range=fall_point_range)
                continue
            elif choice == "cs":
                # '''
                direction = input("Enter the direction (anti or clock): ")
                ...
                self.ocr.diamond_target_time = int(
                    input("Enter the lap time the computer needs to see to click the diamond targeting button: "))
                continue
            elif choice == "ra":
        ...
        while self.is_running:
            ...
            if self.databot_mode:
                ...
                if self.direction == "anticlockwise":
                    ...
                    new_direction = "cw"
                self.scatter.add_data(direction=new_direction, raw=self.raw, winning=winning_number,
                                      rotor_speed=rotor_speed, fall_zone=fall_zone, ball_revs=ball_revs)
                self.scatter.save_profile(SCATTER_DATA_FILE)
            ...
            # tuned_predictions = self.clickbot.get_tuned_from_raw_using_rotor_isolation(direction, rotor_speed,
            # raw_prediction)
            tuned_predictions = self.clickbot.get_tuned_from_raw_using_rotor_isolation(direction, rotor_speed,
                                                                                       adjusted_raw)
            ...
            if not self.databot_mode:
                # predictions have been sent, now wait for the fall zone and ball rev info to come back if NOT in
                # databot mode
                while self.is_running:
                    time.sleep(.05)
                    if self.ocr.quit:
    ...
    def add_data_to_most_recent(self, direction, fall_zone, ball_revs, rotor_speed):
        datapoint = SpinData(direction, Scatter.convert_fall_point_to_diamond_hit(fall_zone, direction), ball_revs,
                             rotor_speed)
